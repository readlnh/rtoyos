...
;中断服务程序
isr_common_stub:
    pusha           ;push all,即push edi, ebp, esp, ebx, edx, ecx, eax
    mov ax, ds
    push eax        ;保存数据段描述符

    mov ax, 0x10    ;加载内核数据段描述段
    mov ds, ax
    mov es, ax 
    mov fs, ax
    mov gs, ax
    ...
    call isr_handler;c语言代码，参数来自栈里(就是esp)
    add esp, 4      ;清除压入的参数
    ...
    popa            ;pop all,即pop edi, ebp, esp, ebx, edx, ecx, eax
